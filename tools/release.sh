...
#                                        ingebakken)
#
# payload voor een loader (geen boot-medium — dit board boot van de kabel):
#   hopos-m4-headless.img.gz             Mac mini M4, via m1n1 (image/apple/
#                                        boot-cycle.sh); ook hier is headless de
#                                        enige smaak — geen framebuffer onder m1n1
#
...
# 3d. Apple silicon (Mac mini M4): geen dd-baar medium — dit board boot niet van
#     een kaart maar van de kabel (iBoot → m1n1 → de proxy schrijft het image op
#     zijn linkadres en springt). Het asset is dus de PAYLOAD, geen boot-medium,
#     en de config reist met de loader mee (CFG=... image/apple/load-probe.py)
#     in plaats van in het image te zitten: er is geen bestandssysteem om hem
#     later te bewerken.
#
#     -headless is hier de enige smaak, en dat is geen keuze maar het silicium:
#     de display-firmware (DCP) komt onder m1n1 op M4 niet op, dus er is geen
#     framebuffer om een desktop op te zetten (board/apple/apple.go). Zelfde
#     naamgeving als de LicheeRV: de smaak staat in de naam, ook al is er maar
#     één.
#
...
if [ -d "${TAMAGO_FORK:-$DIR/../tamago}" ]; then
	echo ">> hopos-m4-headless.img.gz (Apple M4, m1n1-payload)" >&2
	AGENT=1 "$DIR/image/apple-m4.sh" >/dev/null
...
- **hopos-m4-headless.img.gz** — Mac mini M4 (Apple silicon). Not a boot medium: this board boots off the cable, so the image is an **m1n1 payload** — load it with \`image/apple/boot-cycle.sh\`, which needs [m1n1](https://github.com/AsahiLinux/m1n1) on the machine and passes your config along (\`CFG=~/my-node.cfg\`). Headless is the only flavour here too — the display firmware does not come up under m1n1 on M4, so there is no framebuffer.
