...
| patch | wat | waarom |
|---|---|---|
| `0001-runtime-idlehook.patch` | `runtime.IdleMayReady`, `NextTimer` en `RunIdleTimers` (nieuw bestand), `preemptM` op de `goos.Wake`-haak en `preemptone` die ook een idle M kickt | Een SMP-app: `IdleMayReady` zegt of de idle-hook goroutines runnable mag maken (alleen in de scheduler-idle; vanuit `semasleep` zou dat de scheduler slopen). `NextTimer`/`RunIdleTimers` zijn de sysmon-lite van de idle-hook: tamago heeft geen sysmon, dus een timer op een andere P (een gemigreerde pomp reset zijn slaap-timer waar die ligt) draait de hook zelf of kickt de eigenaar. `preemptM`/`preemptone`: een stop-the-world bereikt een core die op EL2 slaapt via dezelfde reschedule-IPI als `semawakeup`, ook een idle M — tamago's idle houdt zijn P vast. Zie `metal/cpu/idle` en `metal/cpu/smp`. |
