...
- [ ] **2.2.6 O6N and Altra sticks are dead for apps (L83 point 34): rebuild as 2.2.7 with the `uefi-run.sh` fix (no `GIC_IPI` off QEMU) and the O6N config without `hopos.nvmewipe`.** Then the wire round on the fresh sticks.
