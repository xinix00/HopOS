...
LicheeRV declares its original 32 MiB kernel window reusable. FLIP prefers that original address when the complete destination is free; if an app owns it, FLIP chooses another free window. A second flip can therefore restore the original placement geometry. Hardware verification covers four swaps on one persistent app TCP connection, returning to the original window at swaps two and four, followed by the full 206 MiB cloudflared/Stulp/plugins workload. Boards without an explicit reservation-release policy retain their fixed cold reservations. The UEFI ownership policy is described below. Total free bytes still do not guarantee a sufficiently large contiguous destination.
...
A flipped kernel enters without firmware: the UEFI stub sees `x1 = 0` and goes straight to the generic boot path. The cold boot publishes the system table, memory map, GOP framebuffer and configuration into its firmware-facts block. `BoardHandoff` copies these facts into the incoming kernel's carve; `hwinit1` restores them before ACPI, pool and configuration initialization. Each replacement reserves the full 160 MiB footprint: 128 MiB of Go RAM and a 32 MiB carve.

The original app-core administration has one persistent owner. A tagged, versioned field in the firmware facts carries the original carve's address through every replacement. `CagePA` keeps referring to its park loops, mailboxes, schedules, contexts and switch code. The incoming kernel gets its own core-zero vectors, node control, DMA buffers and scratch. Pool reconstruction excludes the current kernel and that one original carve; it does not reserve another predecessor carve on each flip.

The original 128 MiB of Go RAM returns to the pool. Its 32 MiB carve remains reserved for cage administration and O6N USB DMA, so the complete original 160 MiB window is not reusable. Intermediate kernel windows return in full. Kernels with older, unversioned firmware facts require a cold entry into this implementation before repeated replacements; the incoming kernel rejects an unproven administration origin. Subsequent UEFI/O6N FLIP targets must implement this same ownership contract. Older bundles are cold-boot-only; the unchanged general FLIP ABI does not itself enforce this board-specific target restriction.

Validation in [L77](v1/technical/release-logboek.md) covers four consecutive QEMU replacements, including a surviving app and TCP connection, followed by stop, an empty replacement with a parked core, and partition reuse. Physical O6N acceptance now covers three compute/TCP resident replacements plus two later GUI/input replacements; see [the follow-up record](v1/technical/release-evidence/2026-09-18-o6n-usb.json). Framebuffer ownership is reconstructed from the preserved, kernel-owned stage-2 mapping before services start. The incoming kernel defers its framebuffer console until it knows whether a resident display owns the screen. The display input stream uses five-second empty-line heartbeats and a fifteen-second read deadline to reconnect after its node-side connection is replaced.
