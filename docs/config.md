...
| `hopos.net.buffer` | total HOP-owned network arena; accepts bytes or `K`/`M`/`G` suffixes. HOP first places the fixed per-slot control and descriptor pages, then manages every remaining 2 KiB chunk as one dynamically borrowed frame pool. No payload is pre-assigned to a slot, and changing this capacity does not change the app ABI | `50M`; LicheeRV `4M` |
...
| `hopos.reboot` | `1` = this image is a reboot request: it fires the hardware watchdog right after the flip report and the node comes back in its installed image. Flip a bundle built with this setting to reboot a node remotely (Apple: `apple.WDTReboot`; other boards arm the watchdog and stop petting). Do not chain reboots: an Apple node that fails to boot cleanly several times in a row drops into recovery mode | off |
| `hopos.idleyield` | `1` = yield-idle for every app, also on a board whose `Cores.IdleMode` does not ask for it. A test aid: on QEMU the rotation spins (WFE does not sleep there), so the sibling-wake path (HVC #4) can be proven without an IPI | off |
