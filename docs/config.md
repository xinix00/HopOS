...
| `hopos.net.buffer` | total HOP-owned pool for every slot's control block and TX/RX rings; accepts bytes or `K`/`M`/`G` suffixes. Ring capacity is discovered at runtime, so changing this does not change the app ABI | `50M`; LicheeRV `4M` |
