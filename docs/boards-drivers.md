...
| Ampere Altra / AmpereOne | UEFI stub retains ACPI, memory-map and GOP information; EL2 required | MADT CPU topology and PSCI; reserved kernel/carve window, app pool checked against the firmware memory map. The [UEFI layer](../metal/board/uefi/hop/board.go) and [plan](../metal/board/uefi/plan.go) are shared; the [Altra board](../metal/board/altra/hop/board.go) adds the `igb` adapter and SMpro temperature. |
...
| UEFI ARM64 | The adapter its board registers — Intel `igb` on Altra and the EDK2 test machine, Realtek `rtl8126` on the Orion O6N — through firmware-configured PCIe; first supported port | [MCFG hierarchy NVMe discovery](../metal/board/uefi/hop/storage.go), including controllers behind root ports; whole selected device assigned to HopOS | GOP framebuffer if supplied by firmware |
