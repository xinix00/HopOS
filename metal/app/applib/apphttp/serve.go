...
func readRequest(c *conn) (*Request, error) {
	...
	if !ok {
		return nil, badRequest("apphttp: malformed request line %q", line)
	}
	...
	if !strings.HasPrefix(proto, "HTTP/1.") {
		return nil, parseError{505, fmt.Sprintf("apphttp: unsupported protocol %q", proto)}
	}
	u, err := url.ParseRequestURI(target)
	if err != nil {
		return nil, badRequest("apphttp: bad request target %q", target)
	}
	...
	for {
		...
		if err != nil {
			...
			return nil, badRequest("apphttp: read headers: %v", err)
		}
		if l == "" {
			break
		}
		...
		if !found {
			return nil, badRequest("apphttp: malformed header %q", l)
		}
	...
	case hdr.Get("Content-Length") != "":
		...
		if err != nil || n < 0 {
			return nil, badRequest("apphttp: bad Content-Length %q", hdr.Get("Content-Length"))
		}
		if n > maxBodyBytes {
			return nil, parseError{StatusRequestEntityTooLarge,
				fmt.Sprintf("apphttp: body of %d bytes exceeds the %d-byte limit", n, maxBodyBytes)}
		}
...
var errHijacked = errors.New("apphttp: connection was hijacked")
...
func (w *respWriter) Hijack() (net.Conn, *bufio.ReadWriter, error) {
	if w.started {
		return nil, nil, errors.New("apphttp: Hijack after the response already started")
	}
	if w.c.hijacked {
