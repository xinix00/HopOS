...
func idleStat() {
	...
	for {
		...
		fmt.Printf("idle: %.0f wakes/s, %.1f%% idle, %.0f irq/s, %.0f empty rx rounds/s; waker %.0f rounds/s, asleep %.0f/s, fallback kicks %.0f/s, direct-rx %.0f/s; switch wakes %.0f/s; app cores %.0f el2 sleeps/s HOPOS_IDLESTAT\n",
			...
			float64(sw1-sw0)/dt, float64(s1-s0)/dt)
		fmt.Printf("idle: cores%s\n", slots.CoreDump())
		w0, t0, i0, r0, at = w1, t1, i1, r1, now
		wr0, wa0, wk0, dk0, sw0, s0 = wr1, wa1, wk1, dk1, sw1, s1
	}
}
